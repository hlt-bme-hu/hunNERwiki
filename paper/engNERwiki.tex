...
To our knowledge, such type of automatically built corpora are not freely available, with the only one exception of the Semantically Annotated Snapshot of the English Wikipedia \cite{Zaragoza:07}. It provides a wide range of linguistic information: POS tags, dependency labels, WordNet super senses and NE annotation according to WSJ and CoNLL tagsets. However, automatically NE-annotated corpus for Hungarian does not exist. Now we present freely available, automatically NE-tagged corpora for English and Hungarian. 
