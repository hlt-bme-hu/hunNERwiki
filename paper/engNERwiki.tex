...
Strictly speaking, our original assumption of equating Wikipedia articles with named entities is not valid: many pages describe common nouns (Book, Aircraft), numbers (1, 42), currencies (Euro, Dollar) or other concepts that fall outside the scope of NER. In this section, we describe the measures taken to avoid errors introduced by these false entities.

Links to number pages are filtered by the surface form of the page title. To Common nouns are filtered by their surface form and POS category: links that contain no proper nouns are discarded

Proper entities However, such entities are either missing from DBpedia altogether, or we can map the classes they belong to to the NER category \texttt{0} (not an entity). This "ontological filtering" ensures that 

Dates require special attention because 
