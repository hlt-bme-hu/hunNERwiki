...
Using the mapping described above, we compile a NE list that contains all entities in DBpedia tagged with the appropriate CoNLL category.
...
Our approach to entity identification relies on Wikipedia cross-references found in the article text. We assume that individual Wikipedia articles describe NEs. A link to an article can then be perceived as a mapping that identifies its anchor text with a particular entity.
