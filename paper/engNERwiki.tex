...
Wikipedia proves to be a rich source of information about its entities; aside from the article text, a huge amount of data is encompassed in infoboxes, templates and the category structure. Since our goal is to create a NER training corpus, our interest is limited to the article text. Templates are removed from the articles; however, the names of the templates have been retained for , but all data (including infobox) 

Beside regular articles, Wikipedia also contains redirect and disambiguation pages, which require special treatment. Redirect pages are empty and can be safely ignored altogether. Disambiguation pages contain text, usually in the form of an enumeration of page links with a short description. Full sentences are scarce and the structure of these pages makes it hard to extract valid training sentences from them. Therefore, disambiguation pages have been dropped from the corpus as well.

The corpus is based on the Wikipedia snapshot as of XXX. The xml files were parsed with the XXX (Attila) parser. The raw text was tokenized with a modified version of the XXX sentence tokenizer in NLTK (CITE). The data was then converted into the tab separated format used by ConLL (XXX, CITE).

Though not included in the corpus, redirect pages can improve recall of the gold standard (XXX). 
% 1. Parsing Wikipedia (hunpos)
% TODO: redirect redirect pages

\subsection{The DBpedia -- ConLL mapping}

The entities in DBpedia are organized into a type hierarchy, available as an OWL (CITE) ontology. The taxonomy 
% description of the DBpedia ontology
On the surface, this hierarchy seems the parallel the four categories (\texttt{PER, LOC, ORG and MISC}) XXX. However, closer inspection reveals that the similarity is superficial. The ConLL NER annotation guide (CITE?: http://www.cnts.ua.ac.be/conll2003/ner/annotation.txt)
